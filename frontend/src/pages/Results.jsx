...
const Results = () => {
    ...
    const handleUpdateTested = async (id) => {
        try {
            const testedPatient = await http.post(`/api/bktest/updateBookTestById/${id}`)
            console.log("Updated");
        } catch (error) {
            console.error(error);
        }
        ...
        try {
            ...
            const addFullName = await Promise.all(
                bookedData.map(async (booking) => {
                    const userResponse = await http.get(`/user/getUserByEmail/${booking.user_email}`);
        ...
        <div className='flex flex-col items-center h-4/5 bg-teal-50 p-auto'>
            <div className='flex flex-col m-3 md:flex-row'>
                        ...
                        booked.map((booking) => (
                            ...
                            <div className='flex flex-col justify-center border items-center p-2 mt-3'>
                                ...
                                <div className='p-2 border m-2'>
                                    <h2>Name: <span className='text-green-700'>{booking.full_name}</span></h2>

                                    <h2>Test: <span className='text-green-700'>{booking.disease}</span></h2>

                                    <h2>Booking No: <span className='text-green-700'>{booking.booking_no}</span></h2>
                                </div>
                                <div className='flex flex-row w-full border justify-center text-white p-1'>
                                    <button 
                                        type="button"
                                        className='bg-green-700 w-1/2 mr-1'
                                        onClick={() => handleTestButtonClick(booking._id)} >Test</button>
                                    <button
                                        type="button"
                ...
                <div className='w-full border ml-3 md:w-1/2'>
                    <h1 className="text-center text-green-700 text-2xl mb-4">Edit Results</h1>
                    <form className='flex flex-col p-2' action="">
                        <input className='m-2 p-2 w-4/5 border-b-2' type="text" placeholder='Test No' name='test_no' value={formData.test_no} onChange={handleOnChange}/>

                        <select class="block appearance-none w-4/5 bg-white border border-gray-200 text-gray-700 m-2 p-2 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" required name='resuts' value={formData.results} onChange={handleOnChange}>
                        ...
                        <div className='flex justify-end w-4/5'>
                            <input className='bg-green-700 text-white p-2 w-1/2 rounded-md' type="button" value="Save" onClick={handleSave}/>
                        </div>                        
                    </form>
                </div>
            </div>
