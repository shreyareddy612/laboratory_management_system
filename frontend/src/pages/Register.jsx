...
const Register = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await axios.post('/api/user/registerUser', formData, options);
            navigate("/login");
        } catch (error) {
            console.error(error);
        }
    ...
    return (
        <div className='flex flex-col justify-center items-center h-4/5 bg-teal-50'>
            <form className="flex flex-col items-center rounded-lg shadow-md p-2 w-full sm:w-2/3 md:w-1/2 lg:w-1/3">
                ...
                <input className="border-b-2 border-gray-400 w-4/5 p-2 mb-4 focus:outline-none focus:border-green-700" required type="text" placeholder="Full Name" name='full_name' onChange={handleFormChange} />
                ...
                <button className="bg-green-700 hover:bg-green-600 text-white font-bold p-2 rounded focus:outline-none focus:shadow-outline w-4/5 mb-4" type="submit" onClick={handleSubmit}>Register</button>
